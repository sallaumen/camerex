defmodule Camerex.Pipeline.LayerRunner do
  ...
  # Hair tem precedência hair_model (mapa) > hair_color (tupla/lista); outros
  # leem #{id}_color direto. :auto (Skin) e :none (Object) não pegam cor.
  defp resolve_color(%{id: :hair}, params),
    do: Map.get(params, "hair_model") || Map.get(params, "hair_color")

  defp resolve_color(%{color_mode: m}, _params) when m in [:none, :auto], do: nil
  defp resolve_color(%{id: id}, params), do: Map.get(params, "#{id}_color")
  ...
  defp sensitivity(%{id: id}, params) do
    case Map.get(params, "#{id}_sensitivity") do
